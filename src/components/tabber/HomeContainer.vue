<template>
    <div>
        <!--这是轮播图区域 -->
        <lunbotu :lunbotuList="lunbotuList" :isFull="isFull"></lunbotu>
...
export default {
    ...
    methods:{
        getLunbotu(){
            //获取轮播图的方法
            this.$http.get("http://localhost:8080/src/images/lunbotu/lunbotu.txt").then(result => {
               if(result.status === 200){
                    var path = result.body.split(",");
                   for(var i=1 ; i<path.length-1 ;i++){
                       this.lunbotuList.push(path[i]);
                   }    
               }else{
                   //失败的
                Toast({
